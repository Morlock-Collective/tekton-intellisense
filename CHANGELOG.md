...
### Added
- Cluster-shared resources: reference a Task/Pipeline/etc. defined only on the cluster (`tektonIntellisense.clusterResources.*` settings, `Tekton: Configure/Refresh Cluster Resources`, `Tekton: Authenticate to Cluster`) — resolves for completion/hover/Go to Definition/diagnostics, read-only; a refresh triggered by a settings change reports success/failure visibly rather than only logging it, and a configured `command` that can't be found at all (a shell alias, commonly) gets one clear message instead of a raw `ENOENT`
